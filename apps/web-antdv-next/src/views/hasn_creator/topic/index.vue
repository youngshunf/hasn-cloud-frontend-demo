...
import type { Topic, TopicParams } from '#/api/hasn_creator/topic';
...
const [editModal, editModalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    if (valid) {
      editModalApi.lock();
      const data = await editFormApi.getValues<TopicParams>();
      try {
        await updateTopicApi(editId.value, data);
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      const data = editModalApi.getData<Topic>();
...
const [addModal, addModalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    if (valid) {
      addModalApi.lock();
      const data = await addFormApi.getValues<TopicParams>();
      try {
